...
Leader::Leader(void)
{
	mHeight = LEADER_TEX_HEIGHT;
	mWidth = LEADER_TEX_WIDTH;
...
void Leader::Load(){
	...
	sf::Texture *idleTex = mAssetManager->GetTexture(std::string("..\\Assets\\leader_idle.png"));
	...
	//run
	sf::Texture *runTex = mAssetManager->GetTexture(std::string("..\\Assets\\leader_walk.png"));
	...
	//death
	sf::Texture *deathTex = mAssetManager->GetTexture(std::string("..\\Assets\\leader_die.png"));
...
sf::Rect<float> Leader::GetBoundingBox(){
	sf::FloatRect bb;
	bb = sf::FloatRect( mPosition.x , mPosition.y, (float)mWidth, (float)mHeight );

	//shrink box to better fit a leader
	bb.height = 88; //height of about 80px
	bb.top += 40; //offset of 40

	bb.width = 60; //texture is about 60px wide
	bb.left += 4; //texture has an edge of about 4px
...
sf::Vector2f Leader::GetDimensions(){
	return sf::Vector2f( (float)mWidth, (float)mHeight );
}



void Leader::Update(GameStateManager &manager, float deltaTime){
		...
		case ES_RUNNING:{
			...
			switch (mFacing){
				...
				case DIR_RIGHT:
					...
					if( (x+1 < (int)(mLevelData->GetLevelDimensions().x/tileDims.x) ) 
						&& (world[y][x+1] != NULL ) 
					){
						//Wall, change direction
						SetFacing( DIR_LEFT );
...
void Leader::UpdatePhysics(GameStateManager &manager, float fixedDeltaTime){
	bool grounded;
...
//update movement
	...
	if( grounded ){
		...
		switch (mCurrentState){
			...
			case ES_RUNNING:
				mCurrentRunSpeed += LEADER_ACCELERATION * fixedDeltaTime;
				
				if( mCurrentRunSpeed > LEADER_MAX_SPEED ){
					mCurrentRunSpeed = LEADER_MAX_SPEED;
				}
	...
	//Hero is not grounded, apply gravity
		...
		switch (mCurrentState){
			...
			default:
				mCurrentMovementVector.y += LEADER_GRAVITY * fixedDeltaTime;
				//cap gravity
				if (mCurrentMovementVector.y  > LEADER_GRAVITY_MAX){
					mCurrentMovementVector.y = LEADER_GRAVITY_MAX;
				}
			break;
		}
...
void Leader::Draw(GameStateManager &manager, sf::RenderWindow &gameWindow){
	switch( mCurrentState ){
		case ES_IDLE:
...
void Leader::SetState( eEnemyState newstate ){
...
void Leader::SetFacing( eDirection newDir ){
